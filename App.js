...
function addSub(id) {
    ...
    $.get('http://127.0.0.1:5000/checksub=' + value, function(data, status){
        doPost(data,value);
    });
...
function doPost(validity, sub_name) {
    sub_name = sub_name.toLowerCase();
    if (validity == 'valid' && !(subs_array.includes(sub_name))) {
        subs_array.push(sub_name);
        let o1 = {
            subreddits: subs_array
        };
        //let o = JSON.stringify(o1);
        //console.log(o);
        $.post('http://127.0.0.1:5000/getlinks', 
            o1,
...
var height = document.getElementById("DataVis").clientHeight
//console.log(10)
//console.log(document.getElementById("DataVis").clientWidth)
//console.log(document.getElementById("DataVis").clientHeight)
var width = document.getElementById("DataVis").clientWidth
var r = 10
...
function ticked() {
    ...
    nodes
        .attr("cx", function(d) {
            return d.x = Math.max((r+25), Math.min(width - (r+25), d.x));
        })
        .attr("cy", function(d) {
            return d.y = Math.max((r+15), Math.min(height - (r+10), d.y));
        })
...
function reload() {
    insularity = calculateInsularity(Data)
    svg.remove()
